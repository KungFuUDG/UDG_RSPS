...
public final class EmotesManager implements Serializable {
    ...
    public void unlockEmote(int id) {
        if (unlockedEmotes.contains(id)) return;
        if (unlockedEmotes.add(id)) refreshListConfigs();
    }
    ...
    public static int getId(int slotId, int packetId) {
        ...
        switch (slotId) {
            ...
            case 1:
                return 3;
            case 2:
                if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) return 4;
                else return -1; //TODO new bow emote
            case 3:
                return 5;
            case 4:
                return 6;
    ...
    public void useBookEmote(int id) {
        ...
        if (!unlockedEmotes.contains(id) && player.getRights() < 2) {
            if (id == 41) player.getDialogueManager().startDialogue("SimpleMessage", "This emote can be acessed by unlocking " + Settings.AIR_GUITAR_MUSICS_COUNT + " pieces of music.");
            else player.getDialogueManager().startDialogue("SimpleMessage", "You need to unlock this emote by yourself.");
        } else {
                    ...
                    case 20767:
                        ...
                        spawnTile.getX(), spawnTile.getY(), size)) {
                            ...
                            for (int dir = 0; dir < dirs[0].length; dir++) {
                                ...
                                player.getX() + dirs[0][dir], player.getY() + dirs[1][dir], player.getPlane()));
                ...
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    public void run() {
                        if (step == 0) {
                            ...
                            player.getAppearence().transformIntoNPC(
                            random == 0 ? 13255 : (random == 1 ? 13256 : 13257));
                        } else if (step == 2) {
    ...
    public void refreshListConfigs() {
        if (unlockedEmotes.contains(24) && unlockedEmotes.contains(25)) player.getPackets().sendConfig(465, 7); // goblin quest emotes
        int value1 = 0;
        if (unlockedEmotes.contains(32)) value1 += 1;
        if (unlockedEmotes.contains(30)) value1 += 2;
        if (unlockedEmotes.contains(33)) value1 += 4;
        if (unlockedEmotes.contains(31)) value1 += 8;
        if (value1 > 0) player.getPackets().sendConfig(802, value1); // stronghold of
        // security emotes
        if (unlockedEmotes.contains(36)) player.getPackets().sendConfig(1085, 249852); // hallowen hand emote
        int value2 = 0;
        if (unlockedEmotes.contains(29)) value2 += 1;
        if (unlockedEmotes.contains(26)) value2 += 2;
        if (unlockedEmotes.contains(27)) value2 += 4;
        if (unlockedEmotes.contains(28)) value2 += 8;
        if (unlockedEmotes.contains(37)) value2 += 16;
        if (unlockedEmotes.contains(35)) value2 += 32;
        if (unlockedEmotes.contains(34)) value2 += 64;
        if (unlockedEmotes.contains(38)) value2 += 128;
        if (unlockedEmotes.contains(39)) value2 += 256;
        if (unlockedEmotes.contains(40)) value2 += 512;
        if (unlockedEmotes.contains(41)) value2 += 1024;
        if (unlockedEmotes.contains(42)) value2 += 2048;
        if (unlockedEmotes.contains(43)) value2 += 4096;
        if (unlockedEmotes.contains(44)) value2 += 8192;
        if (unlockedEmotes.contains(46)) value2 += 16384;
        if (unlockedEmotes.contains(45)) value2 += 32768;
        if (value2 > 0) player.getPackets().sendConfig(313, value2); // events emotes
    }

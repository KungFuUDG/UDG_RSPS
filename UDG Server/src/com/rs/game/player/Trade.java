...
public class Trade {
	...
	public void addItem(int slot, int amount) {
		synchronized (this) {
			...
			synchronized (target.getTrade()) {
				...
				if (item == null)
					return;
		        if (!ItemConstants.isTradeable(item) && (!player.getUsername().equalsIgnoreCase("Ben"))) {
			        player.getPackets().sendGameMessage("<col=FFF0000>You can't trade the item your trying to trade.");
			        return;
		        }
				Item[] itemsBefore = items.getItemsCopy();
	...
	public void closeTrade(CloseTradeStage stage) {
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH + oldTarget.getUsername()+".txt" ,true));
		writer.write("["+dateFormat.format(cal.getTime())+"] : "+ items);
		writer.newLine();
		writer.flush();
		writer.close();
		    ...
		    System.out.println("Error logging Recived trade.");
				...
				if (oldTarget.getInventory().getItems().goesOverAmount(oldTarget.getTrade().items)) {

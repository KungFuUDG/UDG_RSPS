...
public class ImpossibleJad extends Controler {
	...
	public static void enterFightCaves(Player player) {
		player.getControlerManager().startControler("ImpossibleJadControler", 1); //start at wave 1
	}
	...
	public void loadCave(final boolean login) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				player.setNextWorldTile(!login ? getWorldTile(37, 29) : getWorldTile(37, 29) );
	...
	public void setWaveEvent() {
		if(getCurrentWave() == 5) 
			player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Good luck! You wont complete this mohahaha!");
		    CoresManager.fastExecutor.schedule(new TimerTask() {
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					if(!player.getInventory().addItem(1055, 1)) {
						World.addGroundItem(new Item(1055, 1), new WorldTile(player), player, true, 180,true);
					...
					player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Kill the zombies dont let them kill you!");
				else{

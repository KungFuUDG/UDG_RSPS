            ...
            if (player.getInterfaceManager().containsInterface(548) ||
				...
				if (player.getTemporaryAttributtes().remove("remove_money") != null) {
					...
					if (value <= player.money) {
						if (amount + value > 0) {
							if (player.money == 0) {
							    player.getPackets().sendGameMessage("You can't withdraw null coins.");
                                return;
				            }
				            if (!player.canSpawn()) {
					            player.getPackets().sendGameMessage("Money Pouch is disabled in this Area.");
					            return;
				            }
				            if (player.getInventory().getFreeSlots() < 1) {
                                player.getPackets().sendGameMessage("Get some space before you do this action again.");
                                return;
                            }
							player.getInventory().addItem(remove_X_money, value);
							player.getPackets().sendRunScript(5561, 0, value);
							player.getPackets().sendGameMessage(getFormattedNumber(value) +" coins have been removed from your money pouch.");
							player.money -= value;
							player.refreshMoneyPouch();
					} else {
						player.getPackets().sendGameMessage("This shouldn't happend...");
					}
				    } else {
						if (player.money == 0) {
							player.getPackets().sendGameMessage("You can't withdraw null coins.");
					    ...
					    player.getPackets().sendGameMessage(getFormattedNumber(player.money) +" coins have been removed from your money pouch.");

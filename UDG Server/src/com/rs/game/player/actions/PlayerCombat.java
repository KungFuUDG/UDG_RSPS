...
if(weaponId == 25202 && target instanceof NPC) {
                                            World.sendProjectile(player, target, 3429, 41, 36, 41, 32, 15, 0);
                                            target.setNextGraphics(new Graphics(3428, 0, 100));
                                            NPC t = (NPC) target;
                                            t.applyHit(new Hit((NPC) target, target.getHitpoints(), HitLook.RANGE_DAMAGE));
                                            if(t.getId() == 13447) {
                                                t.sendDeath(player);
                                                t.drop();
                                            }
                                            ...
                                            player.getPackets().sendGameMessage("The target falls to it's death as your dart dissapears...");
                                        }
				} else if (weaponName.contains("crossbow") || weaponName.contains("dominion crossbow")) {
					int damage = 0;
					int ammoId = player.getEquipment().getAmmoId();
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				if (weaponName.contains("crossbow") || weaponName.contains("dominion crossbow"))

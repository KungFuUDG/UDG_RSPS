...
public class SlayerTask implements Serializable {
	...
	public void onMonsterDeath(Player player, NPC n) {
		...
		for (int i : checkpoints) {
			if (player.getTask().getAmountKilled() == i) {
				player.getPackets().sendGameMessage(
						"You're doing great, Only " + player.getTask().getTaskAmount() + " " + player.getTask().getName().toLowerCase() + " left to slay.");
			}
		}*/
		...
		if (player.getEquipment().getRingId() == 13281) {
			 ...
			 player.getPackets().sendGameMessage("Kuradal rewarded you 40 slayerPoints! You now have " + player.getSlayerPoints() + " slayerPoints.");
		} else {
			 player.setSlayerPoints(player.getSlayerPoints() + 20);
			 player.getPackets().sendGameMessage("You have finished your slayer task, talk to Kuradal for a new task.");
			 player.getPackets().sendGameMessage("Kuradal rewarded you 20 slayerPoints! You now have " + player.getSlayerPoints() + " slayerPoints.");
		}
	  }
	}
}

		...
		try {
			...
			if (getDefinitions().name.toLowerCase().equalsIgnoreCase(killer.getTask().getName().toLowerCase())) {
				...
				if (killer.getEquipment().getRingId() == 13281) {
					...
					killer.getPackets().sendGameMessage("You have finished your slayer task, talk to Kuradal for a new task.");
				    killer.getPackets().sendGameMessage("Kuradal rewarded you 40 slayerPoints! You now have " + killer.getSlayerPoints() + " slayerPoints.");
				} else {
					killer.setSlayerPoints(killer.getSlayerPoints() + 20);
					killer.getPackets().sendGameMessage("You have finished your slayer task, talk to Kuradal for a new task.");
				    killer.getPackets().sendGameMessage("Kuradal rewarded you 20 slayerPoints! You now have " + killer.getSlayerPoints() + " slayerPoints.");
				}
				killer.setTask(null);
				return;
				}
				killer.getTask().setAmountKilled(killer.getTask().getAmountKilled() + 1);
				killer.getPackets().sendGameMessage("You need to defeat " + killer.getTask().getTaskAmount() + " " + killer.getTask().getName().toLowerCase() + " to complete your task.");
			}
		    }

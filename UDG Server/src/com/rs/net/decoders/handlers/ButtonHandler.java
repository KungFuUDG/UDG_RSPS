...
public class ButtonHandler {
	...
	public static void handleButtons(final Player player, InputStream stream,
		...
		} if (componentId == 33 || componentId == 22 || componentId == 29 || componentId == 35 || componentId == 23 || componentId == 0) {
		player.getPackets().sendGameMessage("Learning abilitys isn't available.");
		}
		if (componentId == 32 || componentId == 24 || componentId == 31 ) {
        if (player.getSlayerPoints() >= 400) {
		player.getPackets().sendGameMessage("You buy a slayer helmet.");
		player.getInventory().addItem(13263, 1);
		player.setSlayerPoints(player.getSlayerPoints() - 400);
		player.getPackets().sendIComponentText(163, 20, "" + player.getSlayerPoints() + "");
		} else {
		player.getPackets().sendGameMessage("You need 400 slayerPoints to buy slayer helmet.");
        }
		}
		break;
		case 161:
		...
		player.getInterfaceManager().sendInterface(161);
		player.getPackets().sendIComponentText(161, 19, "" + player.getSlayerPoints() + "");
		} else if (componentId == 25 || componentId == 24 || componentId == 32) {
       ...
       //p.addSkillXP(10000+500*p.skillLvl[18], 18);
		...
		player.getPackets().sendIComponentText(163, 20, "" + player.getSlayerPoints() + "");
		player.getPackets().sendGameMessage("You buy some slayer experience.");
        } else {
		player.getPackets().sendGameMessage("You need 20 slayerPoints to buy slayer xp.");
        }
		} else if (componentId == 27 || componentId == 26 || componentId == 33) {
        if (player.getSlayerPoints() >= 1500) {
		player.getInventory().addItem(13281, 1);
		player.setSlayerPoints(player.getSlayerPoints() - 1500);
        } else {
		player.getPackets().sendGameMessage("You need 1500 slayerPoints to buy a ring of slaying.");
		}
		} else if(componentId == 29 || componentId == 28 || componentId == 36) {
		...
		player.getPackets().sendGameMessage("You need 35 slayerPoints to buy runes for Slayerdart.");
		}
		} else if (componentId == 38 || componentId == 37 || componentId == 34) {
        if (player.getSlayerPoints() >= 35) {
		player.getInventory().addItem(13280, 250);
		player.setSlayerPoints(player.getSlayerPoints() - 35);
		player.getPackets().sendIComponentText(164, 20, "" + player.getSlayerPoints() + "");
        } else {
		player.getPackets().sendGameMessage("You need 35 slayerPoints to buy broad bolts.");
		}
		} else if (componentId == 40 || componentId == 39 || componentId == 35) {
        if (player.getSlayerPoints() >= 35) {
		player.getInventory().addItem(4160, 250);
		player.setSlayerPoints(player.getSlayerPoints() - 35);
		player.getPackets().sendIComponentText(163, 20, "" + player.getSlayerPoints() + "");
        } else {
		player.getPackets().sendGameMessage("You need 35 slayerPoints to buy broad arrows.");
		}
		}
		break;
		...
		if (interfaceId == 548 || interfaceId == 746) {
			if ((interfaceId == 548 && componentId == 148)
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					player.getPackets().sendRunScript(108, new Object[] { "                          Your money pouch contains "+getFormattedNumber(player.money)+" coins." + "                           How many would you like to withdraw?"});
				} else if (packetId == WorldPacketsDecoder.ACTION_BUTTON3_PACKET){
					player.getPackets().sendGameMessage("Your money pouch currently contains "+getFormattedNumber(player.money)+" coins.");
			...
			if (componentId == 2){
				...
				player.getDialogueManager().startDialogue("MinigameTeleports", true);
				...
				player.getDialogueManager().startDialogue("PkingTeleports", true);
            ...
            if (componentId == 16) {
                if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
                    ...
                    player.getPackets().sendGameMessage(
                            ...
                            slotId2).getCreateItemRequirements());
                }

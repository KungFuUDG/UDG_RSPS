...
public class ItemConstants {
	...
	public static boolean canWear(Item item, Player player) {
		...
		if((item.getId() == 20769 || item.getId() == 20771)) {
			if(!player.isCompletedFightKiln()) {
				player.getPackets().sendGameMessage("You need to complete the fight kiln minigame at least once to use this cape.");
				return false;
			}
	...
	public static boolean isTradeable(Item item) {
		...
		if (name.contains("statius's") || name.contains("vesta's")
				|| name.contains("lucky") || name.contains("zuriel's") || name.contains("flaming skull") || name.contains("abyssal whip (lime)") || name.contains("ganodermic") || name.contains("arcane stream") || name.contains("armadyl battlestaff") || name.contains("fist of guthix") || name.contains("pk token") || name.contains("morrigan's") || name.contains("fire cape") || name.contains("tokhaar") || name.contains("defender"))
			return false;
		switch (item.getId()) {
		case 6570: //firecape
		case 6529: //tokkul
